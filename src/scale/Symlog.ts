...
import {
    DimensionName, ParsedAxisBreakList,
    ScaleTick
} from '../util/types';
...
class SymlogScale extends IntervalScale {
    static type = 'symlog';
    ...
    protected _niceExtent: [number, number] = [ Infinity, -Infinity ];
    private _niceExtentValue: [number, number] = [ Infinity, -Infinity ];
    private _extentPrecision: number = Infinity;
    ...
    getScaleTicks(opt?: ScaleGetTicksOpt): ScaleTick[] {
        ...
        if (this.debugflag) {
           console.group("%o: getScaleTicks(expandToNicedExtent %o) interval %o",
                         this._dim, opt.expandToNicedExtent, interval);
        }
        /* eslint-enable */
        ...
        const end = opt.expandToNicedExtent ? niceTickExtent[1] : extent[1];
        ...
        while (tick <= end) {
            ...
            value = roundNumber(value, extentPrecision);
            let lastvalue = this.convertScaleToValue(lasttick);
            lastvalue = roundNumber(lastvalue, extentPrecision);
    ...
    getTicks(opt?: ScaleGetTicksOpt): ScaleTick[] {
        ...
        if (this.debugflag) {
           console.group("%o: getTicks(expandToNicedExtent %o)",
                         this._dim, opt.expandToNicedExtent);
        }
        /* eslint-enable */

        const ticks = this.getScaleTicks(opt);
        ...
        const valueticks = zrUtil.map(ticks, function (tick) {
            ...
            const powVal = roundNumber(value, extentPrecision);
    ...
    setExtent(start: number, end: number): void {
        ...
        if (isFinite(start) && isFinite(end)) {
            const startPrecision = numberUtil.getPrecision(start);
            const endPrecision = numberUtil.getPrecision(end);
            if (startPrecision < endPrecision) {
               this._extentPrecision = startPrecision;
            }
            else {
               this._extentPrecision = endPrecision;
            }
            start = this.convertValueToScale(start);
            end = this.convertValueToScale(end);
        }
